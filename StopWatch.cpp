...
StopWatch::StopWatch() : mStart(clock::now()) {
   static_assert(steady_clock::is_steady, "Serious OS/C++ library issues. Steady clock is not steady");
...
uint64_t StopWatch::ElapsedUs() const {
   return duration_cast<microseconds>(clock::now() - mStart).count();
}

/// @return the elapsed milliseconds since start
uint64_t StopWatch::ElapsedMs() const {
   return duration_cast<milliseconds>(clock::now() - mStart).count();
}

/// @return the elapsed seconds since start
uint64_t StopWatch::ElapsedSec() const {
   return duration_cast<seconds>(clock::now() - mStart).count();
}
/**
 * Resets the start point
 * @return the updated start point
 */
steady_clock::time_point StopWatch::Restart() {
   mStart = clock::now();
   return mStart;
}
