       ...
       std::pair<std::string, std::string> getExePaths()
        {
...
#else
            std::string path("/usr/local/bin/caeinfo");
            bfs::path info_path(path);
            bfs::path vmb_path1(info_path);
            vmb_path1.remove_filename();
            vmb_path1 /= "vmovecae";
            ...
            bfs::path vmb_path2(vmcaefolder_path/"vmovecae");
            bfs::path vmb_path;
            if(bfs::exists(vmb_path2))
                vmb_path = vmb_path2;
            else if(bfs::exists(vmb_path1))
		...
		std::string AddModelImage( const std::vector<std::string>& cae_files)//, std::string& file_type)
        {
            ...
            std::string img_src_dir("reportFiles");
...
bool vcCad2Xml::Convert()
{
	...
	if(g_bOutputXml)
	{
		/*if(m_iXmlFormat == XML_FORMAT::EKM_META_DATA)
		{
			m_sOutputFileName = ("ekm-meta-data.xml");
		}
		else if(m_iXmlFormat == XML_FORMAT::EKM_REPORT)
		{
			m_sOutputFileName = ("ekm-report.xml");
		}
		else if (m_iXmlFormat == XML_FORMAT::EKM_ASM_DEP_EXTRACT)
		{
			m_sOutputFileName = ("minerva-dependency.xml");
		}*/
...
Dtk_ErrorStatus vcCad2Xml::StartDtkAPI()
{
	...
	m_pDtkAPI = Dtk_API::StartAPI(m_sTmpWorkingDir, errorStatus,"VCOLLAB--288V202");
...
Dtk_ErrorStatus vcCad2Xml::WriteXml(Dtk_MainDocPtr inDocument) 
{
	...
	Dtk_string inst_name = RootComponent->InstanceName();
	...
	{ 
		...
		else if(m_iXmlFormat==XML_FORMAT::EKM_REPORT)
		{
			...
			std::string xml_img_str = AddModelImage(cad_files);
								...
								//else
								{
									...
									if (name_it == m_ComponentNameStrArray.end())
									{
										...
										if (asm_file_name != comp_file_name)
										{
											...
											part_path = std::string("\t{ ")+ current_dir + part_path + std::string(" }\t");
...
void vcCad2Xml::WriteXmlNode_StartEnd(std::string tag,std::string sVal, bool bIndent)
{
	std::stringstream ss;
